...
- [x] OpenAI API client: async/await wrapper for vision + chat responses, reads OPENAI_API_KEY from ProcessInfo
- [x] On-task detection: send screen CGImage to OpenAI vision model, classify on-task/off-task/ambiguous with task context
...
- [x] Task verification: send screen CGImage to OpenAI vision model with success criteria, returns verified/not-verified + explanation
