...
struct VerificationResultBody: View {
    @ObservedObject var state: NotchState
    @ObservedObject var session: SessionManager
    @Binding var completionNote: String
    @FocusState.Binding var noteFieldFocused: Bool

    @ViewBuilder
    var body: some View {
        if let result = state.verificationResult {
            content(result)
        }
    }

    @ViewBuilder
    private func content(_ result: VerificationResult) -> some View {
    ...
    @ViewBuilder
    private var notVerifiedContent: some View {
