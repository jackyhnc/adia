...
struct CalloutManagerTests {
    ...
    // MARK: - blockchain keyword tests
    @Test func blockchainKeywordFromSmartContract() {
        #expect(CalloutManager.extractTaskKeyword(from: "writing a smart contract in Solidity for my blockchain class assignment") == "blockchain")
    }
    @Test func blockchainKeywordFromCourseContext() {
        #expect(CalloutManager.extractTaskKeyword(from: "studying for my blockchain course exam on distributed ledger and consensus algorithms") == "blockchain")
    }
    @Test func blockchainKeywordFromWeb3() {
        #expect(CalloutManager.extractTaskKeyword(from: "building a Web3 dApp development project for my blockchain program") == "blockchain")
    }
    @Test func blockchainCalloutsAllThreeTiersNonEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "blockchain", tier: tier)
                #expect(!msgs.isEmpty, "blockchain tier \(tier) must have callouts")
            }
        }
    }
    @Test func blockchainCalloutsTier1HasAtLeastFour() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "blockchain", tier: 1)
            #expect(msgs.count >= 4, "blockchain tier1 must have ≥4 messages")
        }
    }
    @Test func blockchainCalloutsNoneAreEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "blockchain", tier: tier)
                for msg in msgs { #expect(!msg.isEmpty, "blockchain tier \(tier) callout must not be empty") }
            }
        }
    }
    @Test func blockchainCalloutsTier3ContainsCloseThis() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "blockchain", tier: 3)
            let hasCloseThis = msgs.contains { $0.contains("CLOSE THIS") || $0.contains("no one") }
            #expect(hasCloseThis, "blockchain tier 3 must contain 'CLOSE THIS' or 'no one'")
        }
    }
    @Test func blockchainFalsePositiveBareKeyword() {
        let kw = CalloutManager.extractTaskKeyword(from: "bitcoin price is going up")
        #expect(kw != "blockchain", "bare crypto price talk should not route to blockchain")
    }

    // MARK: - digitalmarketing keyword tests
    @Test func digitalmarketingKeywordFromSEO() {
        #expect(CalloutManager.extractTaskKeyword(from: "studying for my SEO certification exam on search engine optimization strategies") == "digitalmarketing")
    }
    @Test func digitalmarketingKeywordFromGoogleAnalytics() {
        #expect(CalloutManager.extractTaskKeyword(from: "completing my Google Analytics certification exam and reviewing data analysis concepts") == "digitalmarketing")
    }
    @Test func digitalmarketingKeywordFromCourse() {
        #expect(CalloutManager.extractTaskKeyword(from: "working on my digital marketing class assignment on content marketing and social media strategy") == "digitalmarketing")
    }
    @Test func digitalmarketingCalloutsAllThreeTiersNonEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "digitalmarketing", tier: tier)
                #expect(!msgs.isEmpty, "digitalmarketing tier \(tier) must have callouts")
            }
        }
    }
    @Test func digitalmarketingCalloutsTier1HasAtLeastFour() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "digitalmarketing", tier: 1)
            #expect(msgs.count >= 4, "digitalmarketing tier1 must have ≥4 messages")
        }
    }
    @Test func digitalmarketingCalloutsNoneAreEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "digitalmarketing", tier: tier)
                for msg in msgs { #expect(!msg.isEmpty, "digitalmarketing tier \(tier) callout must not be empty") }
            }
        }
    }
    @Test func digitalmarketingCalloutsTier3ContainsCloseThis() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "digitalmarketing", tier: 3)
            let hasCloseThis = msgs.contains { $0.contains("CLOSE THIS") || $0.contains("no one") }
            #expect(hasCloseThis, "digitalmarketing tier 3 must contain 'CLOSE THIS' or 'no one'")
        }
    }
    @Test func digitalmarketingFalsePositiveBrandStrategy() {
        let kw = CalloutManager.extractTaskKeyword(from: "developing a brand strategy and brand management plan for my startup pitch deck")
        #expect(kw != "digitalmarketing", "brand strategy / pitch deck should route to startup, not digitalmarketing")
    }

    // MARK: - projectmanagement keyword tests
    @Test func projectmanagementKeywordFromPMP() {
        #expect(CalloutManager.extractTaskKeyword(from: "studying for the PMP exam and reviewing PMBOK knowledge areas") == "projectmanagement")
    }
    @Test func projectmanagementKeywordFromScrum() {
        #expect(CalloutManager.extractTaskKeyword(from: "completing my scrum master certification course on agile project management") == "projectmanagement")
    }
    @Test func projectmanagementKeywordFromCourse() {
        #expect(CalloutManager.extractTaskKeyword(from: "working on my project management class assignment on project charter and WBS") == "projectmanagement")
    }
    @Test func projectmanagementCalloutsAllThreeTiersNonEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "projectmanagement", tier: tier)
                #expect(!msgs.isEmpty, "projectmanagement tier \(tier) must have callouts")
            }
        }
    }
    @Test func projectmanagementCalloutsTier1HasAtLeastFour() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "projectmanagement", tier: 1)
            #expect(msgs.count >= 4, "projectmanagement tier1 must have ≥4 messages")
        }
    }
    @Test func projectmanagementCalloutsNoneAreEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "projectmanagement", tier: tier)
                for msg in msgs { #expect(!msg.isEmpty, "projectmanagement tier \(tier) callout must not be empty") }
            }
        }
    }
    @Test func projectmanagementCalloutsTier3ContainsCloseThis() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "projectmanagement", tier: 3)
            let hasCloseThis = msgs.contains { $0.contains("CLOSE THIS") || $0.contains("no one") }
            #expect(hasCloseThis, "projectmanagement tier 3 must contain 'CLOSE THIS' or 'no one'")
        }
    }
    @Test func projectmanagementFalsePositiveBareProject() {
        let kw = CalloutManager.extractTaskKeyword(from: "working on a project for my coding assignment")
        #expect(kw != "projectmanagement", "bare 'project' with coding context should not route to projectmanagement")
    }

    // MARK: - riskmanagement keyword tests
    @Test func riskmanagementKeywordFromERM() {
        #expect(CalloutManager.extractTaskKeyword(from: "studying enterprise risk management and ERM frameworks for my risk management class exam") == "riskmanagement")
    }
    @Test func riskmanagementKeywordFromRiskAssessment() {
        #expect(CalloutManager.extractTaskKeyword(from: "completing my risk assessment course assignment on risk analysis methodology") == "riskmanagement")
    }
    @Test func riskmanagementKeywordFromGRC() {
        #expect(CalloutManager.extractTaskKeyword(from: "working on my GRC class project on governance risk and compliance frameworks") == "riskmanagement")
    }
    @Test func riskmanagementCalloutsAllThreeTiersNonEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "riskmanagement", tier: tier)
                #expect(!msgs.isEmpty, "riskmanagement tier \(tier) must have callouts")
            }
        }
    }
    @Test func riskmanagementCalloutsTier1HasAtLeastFour() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "riskmanagement", tier: 1)
            #expect(msgs.count >= 4, "riskmanagement tier1 must have ≥4 messages")
        }
    }
    @Test func riskmanagementCalloutsNoneAreEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "riskmanagement", tier: tier)
                for msg in msgs { #expect(!msg.isEmpty, "riskmanagement tier \(tier) callout must not be empty") }
            }
        }
    }
    @Test func riskmanagementCalloutsTier3ContainsCloseThis() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "riskmanagement", tier: 3)
            let hasCloseThis = msgs.contains { $0.contains("CLOSE THIS") || $0.contains("no one") }
            #expect(hasCloseThis, "riskmanagement tier 3 must contain 'CLOSE THIS' or 'no one'")
        }
    }
    @Test func riskmanagementFalsePositiveBareRisk() {
        let kw = CalloutManager.extractTaskKeyword(from: "thinking about the risk of missing the deadline on my code project")
        #expect(kw != "riskmanagement", "bare 'risk' in general context should not route to riskmanagement")
    }

    // MARK: - speechcommunication keyword tests
    @Test func speechcommunicationKeywordFromPublicSpeaking() {
        #expect(CalloutManager.extractTaskKeyword(from: "preparing my speech for my public speaking class presentation") == "speechcommunication")
    }
    @Test func speechcommunicationKeywordFromDebate() {
        #expect(CalloutManager.extractTaskKeyword(from: "prepping my arguments and rebuttals for my debate class competition") == "speechcommunication")
    }
    @Test func speechcommunicationKeywordFromOralCommunication() {
        #expect(CalloutManager.extractTaskKeyword(from: "studying for my oral communication class exam on persuasive speaking") == "speechcommunication")
    }
    @Test func speechcommunicationCalloutsAllThreeTiersNonEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "speechcommunication", tier: tier)
                #expect(!msgs.isEmpty, "speechcommunication tier \(tier) must have callouts")
            }
        }
    }
    @Test func speechcommunicationCalloutsTier1HasAtLeastFour() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "speechcommunication", tier: 1)
            #expect(msgs.count >= 4, "speechcommunication tier1 must have ≥4 messages")
        }
    }
    @Test func speechcommunicationCalloutsNoneAreEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "speechcommunication", tier: tier)
                for msg in msgs { #expect(!msg.isEmpty, "speechcommunication tier \(tier) callout must not be empty") }
            }
        }
    }
    @Test func speechcommunicationCalloutsTier3ContainsCloseThis() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "speechcommunication", tier: 3)
            let hasCloseThis = msgs.contains { $0.contains("CLOSE THIS") || $0.contains("no one") }
            #expect(hasCloseThis, "speechcommunication tier 3 must contain 'CLOSE THIS' or 'no one'")
        }
    }
    @Test func speechcommunicationFalsePositiveSpeechTherapy() {
        let kw = CalloutManager.extractTaskKeyword(from: "writing up my speech therapy session notes for my clinical placement")
    ...
    // MARK: - performanceanalysis domain
    @Test func performanceanalysisKeywordFromDartfish() {
        let kw = CalloutManager.extractTaskKeyword(from: "analyzing match footage using Dartfish for my sports performance analysis class assignment")
        #expect(kw == "performanceanalysis", "Dartfish + performance analysis class should route to performanceanalysis")
    }
    @Test func performanceanalysisKeywordFromHudl() {
        let kw = CalloutManager.extractTaskKeyword(from: "reviewing game film in Hudl and tagging tactical patterns for coaching analytics")
        #expect(kw == "performanceanalysis", "Hudl + coaching analytics should route to performanceanalysis")
    }
    @Test func performanceanalysisKeywordFromNotational() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing my notational analysis assignment on team sports performance")
        #expect(kw == "performanceanalysis", "notational analysis (sports) should route to performanceanalysis")
    }
    @Test func performanceanalysisCalloutsAllTiersNonEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "performanceanalysis", tier: tier)
                #expect(!msgs.isEmpty, "performanceanalysis tier \(tier) must have messages")
            }
        }
    }
    @Test func performanceanalysisCalloutsTier1HasAtLeastFour() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "performanceanalysis", tier: 1)
            #expect(msgs.count >= 4, "performanceanalysis tier1 must have ≥4 messages")
        }
    }
    @Test func performanceanalysisCalloutsNoneAreEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "performanceanalysis", tier: tier)
                for msg in msgs { #expect(!msg.isEmpty, "performanceanalysis tier \(tier) callout must not be empty") }
            }
        }
    }
    @Test func performanceanalysisCalloutsTier3ContainsCloseThis() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "performanceanalysis", tier: 3)
            let hasCloseThis = msgs.contains { $0.contains("CLOSE THIS") || $0.contains("no one") }
            #expect(hasCloseThis, "performanceanalysis tier 3 must contain 'CLOSE THIS' or 'no one'")
        }
    }
    @Test func performanceanalysisFiresBeforeSportsanalytics() {
        let kw = CalloutManager.extractTaskKeyword(from: "producing my performance analysis report on player tracking and video analysis for my sport science class")
        #expect(kw == "performanceanalysis", "video analysis + performance analysis class should fire performanceanalysis before sportsanalytics")
    }
    @Test func performanceanalysisFalsePositiveGeneralPerformance() {
        let kw = CalloutManager.extractTaskKeyword(from: "working on performance improvements for my web app code")
        #expect(kw != "performanceanalysis", "bare 'performance' in coding context should NOT route to performanceanalysis")
    }

    // MARK: - musicbusiness domain
    @Test func musicbusinessKeywordFromMusicBusiness() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for my music business class exam on publishing deals and royalties")
        #expect(kw == "musicbusiness", "music business class exam should route to musicbusiness")
    }
    @Test func musicbusinessKeywordFromASCAP() {
        let kw = CalloutManager.extractTaskKeyword(from: "researching ASCAP licensing royalties for my music industry course paper")
        #expect(kw == "musicbusiness", "ASCAP licensing + music industry course should route to musicbusiness")
    }
    @Test func musicbusinessKeywordFromMusicPublishing() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing my music publishing class assignment on sync licensing and mechanical royalties")
        #expect(kw == "musicbusiness", "music publishing class should route to musicbusiness")
    }
    @Test func musicbusinessCalloutsAllTiersNonEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "musicbusiness", tier: tier)
                #expect(!msgs.isEmpty, "musicbusiness tier \(tier) must have messages")
            }
        }
    }
    @Test func musicbusinessCalloutsTier1HasAtLeastFour() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "musicbusiness", tier: 1)
            #expect(msgs.count >= 4, "musicbusiness tier1 must have ≥4 messages")
        }
    }
    @Test func musicbusinessCalloutsNoneAreEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "musicbusiness", tier: tier)
                for msg in msgs { #expect(!msg.isEmpty, "musicbusiness tier \(tier) callout must not be empty") }
            }
        }
    }
    @Test func musicbusinessCalloutsTier3ContainsCloseThis() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "musicbusiness", tier: 3)
            let hasCloseThis = msgs.contains { $0.contains("CLOSE THIS") || $0.contains("no one") }
            #expect(hasCloseThis, "musicbusiness tier 3 must contain 'CLOSE THIS' or 'no one'")
        }
    }
    @Test func musicbusinessFiresBeforeMusicproduction() {
        let kw = CalloutManager.extractTaskKeyword(from: "writing my music business degree paper on entertainment law contracts and artist management")
        #expect(kw == "musicbusiness", "music business degree paper should fire musicbusiness before musicproduction")
    }
    @Test func musicbusinessFalsePositiveDAW() {
        let kw = CalloutManager.extractTaskKeyword(from: "mixing my new track in Ableton and mastering the final beat")
        #expect(kw == "musicproduction", "DAW/mixing task should route to musicproduction not musicbusiness")
    }

    // MARK: - dentalanesthesia domain
    @Test func dentalanesthesiaKeywordFromCOMS() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for the COMS exam on sedation protocols and dental anesthesia pharmacology")
        #expect(kw == "dentalanesthesia", "COMS exam + dental anesthesia should route to dentalanesthesia")
    }
    @Test func dentalanesthesiaKeywordFromSedationDentistry() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing my sedation dentistry class assignment on IV sedation and patient monitoring")
        #expect(kw == "dentalanesthesia", "sedation dentistry class should route to dentalanesthesia")
    }
    @Test func dentalanesthesiaKeywordFromDentalAnesthesia() {
        let kw = CalloutManager.extractTaskKeyword(from: "reviewing my dental anesthesia rotation notes on conscious sedation techniques")
        #expect(kw == "dentalanesthesia", "dental anesthesia rotation notes should route to dentalanesthesia")
    }
    @Test func dentalanesthesiaCalloutsAllTiersNonEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "dentalanesthesia", tier: tier)
                #expect(!msgs.isEmpty, "dentalanesthesia tier \(tier) must have messages")
            }
        }
    }
    @Test func dentalanesthesiaCalloutsTier1HasAtLeastFour() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "dentalanesthesia", tier: 1)
            #expect(msgs.count >= 4, "dentalanesthesia tier1 must have ≥4 messages")
        }
    }
    @Test func dentalanesthesiaCalloutsNoneAreEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "dentalanesthesia", tier: tier)
                for msg in msgs { #expect(!msg.isEmpty, "dentalanesthesia tier \(tier) callout must not be empty") }
            }
        }
    }
    @Test func dentalanesthesiaCalloutsTier3ContainsCloseThis() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "dentalanesthesia", tier: 3)
            let hasCloseThis = msgs.contains { $0.contains("CLOSE THIS") || $0.contains("no one") }
            #expect(hasCloseThis, "dentalanesthesia tier 3 must contain 'CLOSE THIS' or 'no one'")
        }
    }
    @Test func dentalanesthesiaFiresBeforeDentalradiology() {
        let kw = CalloutManager.extractTaskKeyword(from: "preparing for my dental anesthesia program exam on IV sedation protocols and COMS board prep")
        #expect(kw == "dentalanesthesia", "dental anesthesia program + COMS should fire dentalanesthesia before dentalradiology")
    }

    // MARK: - palliativecare domain
    @Test func palliativecareKeywordFromCHPN() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for the CHPN exam and reviewing palliative care principles and symptom management")
        #expect(kw == "palliativecare", "CHPN exam + palliative care should route to palliativecare")
    }
    @Test func palliativecareKeywordFromHospiceNursing() {
        let kw = CalloutManager.extractTaskKeyword(from: "writing up my hospice nursing notes and end-of-life care plan for my patient")
        #expect(kw == "palliativecare", "hospice nursing notes + end-of-life care should route to palliativecare")
    }
    @Test func palliativecareKeywordFromPalliativeMedicine() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing my palliative care class assignment on comfort measures and goals of care")
        #expect(kw == "palliativecare", "palliative care class assignment should route to palliativecare")
    }
    @Test func palliativecareCalloutsAllTiersNonEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "palliativecare", tier: tier)
                #expect(!msgs.isEmpty, "palliativecare tier \(tier) must have messages")
            }
        }
    }
    @Test func palliativecareCalloutsTier1HasAtLeastFour() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "palliativecare", tier: 1)
            #expect(msgs.count >= 4, "palliativecare tier1 must have ≥4 messages")
        }
    }
    @Test func palliativecareCalloutsNoneAreEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "palliativecare", tier: tier)
                for msg in msgs { #expect(!msg.isEmpty, "palliativecare tier \(tier) callout must not be empty") }
            }
        }
    }
    @Test func palliativecareCalloutsTier3ContainsCloseThis() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "palliativecare", tier: 3)
            let hasCloseThis = msgs.contains { $0.contains("CLOSE THIS") || $0.contains("no one") }
            #expect(hasCloseThis, "palliativecare tier 3 must contain 'CLOSE THIS' or 'no one'")
        }
    }
    @Test func palliativecareFiresBeforeForensicnursing() {
        let kw = CalloutManager.extractTaskKeyword(from: "preparing for my hospice care class exam on palliative nursing and comfort measures")
        #expect(kw == "palliativecare", "hospice care class + palliative nursing should fire palliativecare before forensicnursing/nursing")
    }

    // MARK: - cognitivescience domain
    @Test func cognitivescienceKeywordFromCogSci() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for my cognitive science class exam on cognitive modeling and language processing")
        #expect(kw == "cognitivescience", "cognitive science class exam should route to cognitivescience")
    }
    @Test func cognitivescienceKeywordFromCogsci() {
        let kw = CalloutManager.extractTaskKeyword(from: "working on my cogsci paper on the computational mind and cognitive architecture")
        #expect(kw == "cognitivescience", "cogsci + computational mind paper should route to cognitivescience")
    }
    @Test func cognitivescienceKeywordFromMindBrain() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for my mind and brain class exam on attention, memory, and perception")
        #expect(kw == "cognitivescience", "mind and brain class exam should route to cognitivescience")
    }
    @Test func cognitivescienceCalloutsAllTiersNonEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "cognitivescience", tier: tier)
                #expect(!msgs.isEmpty, "cognitivescience tier \(tier) must have messages")
            }
        }
    }
    @Test func cognitivescienceCalloutsTier1HasAtLeastFour() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "cognitivescience", tier: 1)
            #expect(msgs.count >= 4, "cognitivescience tier1 must have ≥4 messages")
        }
    }
    @Test func cognitivescienceCalloutsNoneAreEmpty() async {
        await MainActor.run {
            for tier in 1...3 {
                let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "cognitivescience", tier: tier)
                for msg in msgs { #expect(!msg.isEmpty, "cognitivescience tier \(tier) callout must not be empty") }
            }
        }
    }
    @Test func cognitivescienceCalloutsTier3ContainsCloseThis() async {
        await MainActor.run {
            let msgs = CalloutManager.shared.taskAwareCallouts(keyword: "cognitivescience", tier: 3)
            let hasCloseThis = msgs.contains { $0.contains("CLOSE THIS") || $0.contains("no one") }
            #expect(hasCloseThis, "cognitivescience tier 3 must contain 'CLOSE THIS' or 'no one'")
        }
    }
    @Test func cognitivescienceFiresBeforeNeuroscience() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing my cognitive science major paper on mind and brain theories and cognitive modeling")
        #expect(kw == "cognitivescience", "cognitive science major paper should fire cognitivescience before neuroscience")
    }
    @Test func cognitivescienceFalsePositiveCogNeuro() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying cognitive neuroscience and action potentials for my neuroscience class")
        #expect(kw == "neuroscience", "cognitive neuroscience + action potentials should route to neuroscience not cognitivescience")
    }

    // MARK: - Count guard (batch: performanceanalysis/musicbusiness/dentalanesthesia/palliativecare/cognitivescience)
    ...
    @Test func animalassistedtherapyKeywordFromTherapyDog() {
        let kw = CalloutManager.extractTaskKeyword(from: "preparing for my therapy dog handling certification exam and reviewing pet partners protocols")
        #expect(kw == "animalassistedtherapy", "therapy dog handling certification pet partners protocols should route to animalassistedtherapy")
    }
    @Test func animalassistedtherapyKeywordFromAAT() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying animal-assisted therapy (AAT) intervention protocols for my certification program")
        #expect(kw == "animalassistedtherapy", "animal-assisted therapy AAT intervention protocols certification should route to animalassistedtherapy")
    }
    @Test func animalassistedtherapyKeywordFromCanineAssistedTherapy() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing my canine-assisted therapy class assignment on session documentation and ethics")
        #expect(kw == "animalassistedtherapy", "canine-assisted therapy class assignment should route to animalassistedtherapy")
    }
    @Test func animalassistedtherapyKeywordFromEquineAssistedPsychotherapy() {
        let kw = CalloutManager.extractTaskKeyword(from: "reviewing equine-assisted psychotherapy techniques and session planning for my EAP training")
        #expect(kw == "animalassistedtherapy", "equine-assisted psychotherapy EAP training should route to animalassistedtherapy")
    }
    @Test func animalassistedtherapyFalsePositive_equineAssistedRecreation() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying equine-assisted therapy techniques in my recreational therapy certification program")
        #expect(kw != "animalassistedtherapy", "equine-assisted therapy with recreation context should NOT route to animalassistedtherapy (stays in recreationaltherapy)")
    }
    @Test @MainActor func animalassistedtherapyCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "animalassistedtherapy", tier: tier)
            #expect(!msgs.isEmpty, "animalassistedtherapy tier \(tier) must return messages")
        }
    }
    @Test @MainActor func animalassistedtherapyTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "animalassistedtherapy", tier: 1)
        #expect(msgs.count >= 4, "animalassistedtherapy tier 1 must have ≥4 messages")
    }
    @Test @MainActor func animalassistedtherapyTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "animalassistedtherapy", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "animalassistedtherapy tier 3 must contain CLOSE THIS")
    }

    // MARK: - constructionlaw
    @Test func constructionlawKeywordFromAIAContract() {
        let kw = CalloutManager.extractTaskKeyword(from: "analyzing AIA contract provisions and construction dispute resolution for my construction law class")
        #expect(kw == "constructionlaw", "AIA contract construction dispute resolution construction law class should route to constructionlaw")
    }
    @Test func constructionlawKeywordFromMechanicsLien() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying mechanics lien procedures and payment bond requirements for my construction law exam")
        #expect(kw == "constructionlaw", "mechanics lien payment bond construction law exam should route to constructionlaw")
    }
    @Test func constructionlawKeywordFromConstructionLaw() {
        let kw = CalloutManager.extractTaskKeyword(from: "reviewing surety bond law and construction claim analysis for my project delivery law course")
        #expect(kw == "constructionlaw", "surety bond law construction claim analysis should route to constructionlaw")
    }
    @Test func constructionlawKeywordFromConstructionLitigation() {
        let kw = CalloutManager.extractTaskKeyword(from: "writing a construction litigation memo on delay claims and contract breach for my class")
        #expect(kw == "constructionlaw", "construction litigation delay claims contract breach class should route to constructionlaw")
    }
    @Test func constructionlawFalsePositive_constructionManagement() {
        let kw = CalloutManager.extractTaskKeyword(from: "working on my construction management assignment on project scheduling and cost estimation")
        #expect(kw != "constructionlaw", "construction management project scheduling cost estimation should NOT route to constructionlaw")
    }
    @Test @MainActor func constructionlawCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "constructionlaw", tier: tier)
            #expect(!msgs.isEmpty, "constructionlaw tier \(tier) must return messages")
        }
    }
    @Test @MainActor func constructionlawTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "constructionlaw", tier: 1)
        #expect(msgs.count >= 4, "constructionlaw tier 1 must have ≥4 messages")
    }
    @Test @MainActor func constructionlawTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "constructionlaw", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "constructionlaw tier 3 must contain CLOSE THIS")
    }

    // MARK: - healtheconomics
    @Test func healtheconomicsKeywordFromQALY() {
        let kw = CalloutManager.extractTaskKeyword(from: "calculating QALY values and interpreting cost-effectiveness thresholds for my health economics class")
        #expect(kw == "healtheconomics", "QALY cost-effectiveness health economics class should route to healtheconomics")
    }
    @Test func healtheconomicsKeywordFromPharmacoeconomics() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying pharmacoeconomics methods — CEA, CBA, and cost-utility analysis — for my exam")
        #expect(kw == "healtheconomics", "pharmacoeconomics CEA CBA cost-utility analysis exam should route to healtheconomics")
    }
    @Test func healtheconomicsKeywordFromHTA() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing a health technology assessment (HTA) assignment on drug cost-effectiveness for my health economics program")
        #expect(kw == "healtheconomics", "health technology assessment HTA drug cost-effectiveness health economics program should route to healtheconomics")
    }
    @Test func healtheconomicsKeywordFromICER() {
        let kw = CalloutManager.extractTaskKeyword(from: "interpreting ICER ratios and willingness-to-pay thresholds for health interventions in my economics course")
        #expect(kw == "healtheconomics", "ICER ratios willingness to pay health interventions economics course should route to healtheconomics")
    }
    @Test func healtheconomicsFalsePositive_bareEconomics() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying macroeconomics and game theory for my economics midterm exam")
        #expect(kw != "healtheconomics", "macroeconomics game theory economics midterm should NOT route to healtheconomics")
    }
    @Test @MainActor func healtheconomicsCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "healtheconomics", tier: tier)
            #expect(!msgs.isEmpty, "healtheconomics tier \(tier) must return messages")
        }
    }
    @Test @MainActor func healtheconomicsTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "healtheconomics", tier: 1)
        #expect(msgs.count >= 4, "healtheconomics tier 1 must have ≥4 messages")
    }
    @Test @MainActor func healtheconomicsTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "healtheconomics", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "healtheconomics tier 3 must contain CLOSE THIS")
    }

    // MARK: - insurancefinance
    @Test func insurancefinanceKeywordFromCPCU() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for my CPCU designation exam on commercial insurance underwriting and risk management")
        #expect(kw == "insurancefinance", "CPCU designation exam commercial insurance underwriting should route to insurancefinance")
    }
    @Test func insurancefinanceKeywordFromInsuranceLicensing() {
        let kw = CalloutManager.extractTaskKeyword(from: "preparing for my state insurance licensing exam on life and health insurance principles")
        #expect(kw == "insurancefinance", "insurance licensing exam life health insurance principles should route to insurancefinance")
    }
    @Test func insurancefinanceKeywordFromPandC() {
        let kw = CalloutManager.extractTaskKeyword(from: "reviewing property and casualty insurance concepts and coverage forms for my P&C insurance class")
        #expect(kw == "insurancefinance", "property and casualty P&C insurance class should route to insurancefinance")
    }
    @Test func insurancefinanceKeywordFromUnderwriting() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing an insurance underwriting case study on risk assessment and premium calculation")
        #expect(kw == "insurancefinance", "insurance underwriting case study risk assessment premium calculation should route to insurancefinance")
    }
    @Test func insurancefinanceFalsePositive_titleInsurance() {
        let kw = CalloutManager.extractTaskKeyword(from: "reviewing title insurance requirements and closing documents for my real estate class")
        #expect(kw != "insurancefinance", "title insurance closing documents real estate class should NOT route to insurancefinance")
    }
    @Test @MainActor func insurancefinanceCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "insurancefinance", tier: tier)
            #expect(!msgs.isEmpty, "insurancefinance tier \(tier) must return messages")
        }
    }
    @Test @MainActor func insurancefinanceTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "insurancefinance", tier: 1)
        #expect(msgs.count >= 4, "insurancefinance tier 1 must have ≥4 messages")
    }
    @Test @MainActor func insurancefinanceTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "insurancefinance", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "insurancefinance tier 3 must contain CLOSE THIS")
    }

    // MARK: - environmentalplanning
    @Test func environmentalplanningKeywordFromEIS() {
        let kw = CalloutManager.extractTaskKeyword(from: "drafting sections of an environmental impact statement (EIS) for my environmental planning class")
        #expect(kw == "environmentalplanning", "environmental impact statement EIS environmental planning class should route to environmentalplanning")
    }
    @Test func environmentalplanningKeywordFromCEQA() {
        let kw = CalloutManager.extractTaskKeyword(from: "reviewing CEQA environmental review procedures and alternatives analysis for my planning exam")
        #expect(kw == "environmentalplanning", "CEQA environmental review procedures alternatives analysis planning exam should route to environmentalplanning")
    }
    @Test func environmentalplanningKeywordFromNEPACompliance() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying NEPA compliance requirements and the scoping process for my environmental planning course")
        #expect(kw == "environmentalplanning", "NEPA compliance scoping environmental planning course should route to environmentalplanning")
    }
    @Test func environmentalplanningKeywordFromPermitting() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing an environmental permitting assignment on EIA process and mitigation monitoring for my land use class")
        #expect(kw == "environmentalplanning", "environmental permitting EIA mitigation monitoring land use class should route to environmentalplanning")
    }
    @Test func environmentalplanningFalsePositive_environmentalScience() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for my environmental science exam on ecology and climate change")
        #expect(kw != "environmentalplanning", "environmental science exam ecology climate change should NOT route to environmentalplanning")
    }
    @Test @MainActor func environmentalplanningCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "environmentalplanning", tier: tier)
            #expect(!msgs.isEmpty, "environmentalplanning tier \(tier) must return messages")
        }
    }
    @Test @MainActor func environmentalplanningTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "environmentalplanning", tier: 1)
        #expect(msgs.count >= 4, "environmentalplanning tier 1 must have ≥4 messages")
    }
    @Test @MainActor func environmentalplanningTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "environmentalplanning", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "environmentalplanning tier 3 must contain CLOSE THIS")
    }
    ...
    @Test func athletictrainingKeywordFromATC() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for my certified athletic trainer exam and reviewing therapeutic modalities for the ATC credential")
        #expect(kw == "athletictraining", "certified athletic trainer ATC credential exam should route to athletictraining")
    }
    @Test func athletictrainingKeywordFromTapingLab() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing my taping and bracing lab report for athletic training class")
        #expect(kw == "athletictraining", "taping and bracing lab athletic training class should route to athletictraining")
    }
    @Test func athletictrainingKeywordFromProgram() {
        let kw = CalloutManager.extractTaskKeyword(from: "working on my athletic training program coursework and injury evaluation class")
        #expect(kw == "athletictraining", "athletic training program coursework should route to athletictraining")
    }
    @Test func athletictrainingKeywordFromModalities() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying therapeutic modalities for my athletic training class — ultrasound, TENS, and cryotherapy")
        #expect(kw == "athletictraining", "therapeutic modalities athletic training class should route to athletictraining")
    }
    @Test func athletictrainingFalsePositive_sportsmedClinical() {
        let kw = CalloutManager.extractTaskKeyword(from: "writing up my clinical athletic training hours for BOC certification documentation")
        #expect(kw == "sportsmedicine" || kw == "athletictraining", "clinical athletic training BOC should stay in sportsmedicine or athletictraining")
    }
    @Test @MainActor func athletictrainingCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "athletictraining", tier: tier)
            #expect(!msgs.isEmpty, "athletictraining tier \(tier) must return messages")
        }
    }
    @Test @MainActor func athletictrainingTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "athletictraining", tier: 1)
        #expect(msgs.count >= 4, "athletictraining tier 1 must have ≥4 messages")
    }
    @Test @MainActor func athletictrainingTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "athletictraining", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "athletictraining tier 3 must contain CLOSE THIS")
    }

    // MARK: - biomechanics
    @Test func biomechanicsKeywordFromForceplate() {
        let kw = CalloutManager.extractTaskKeyword(from: "analyzing force plate data from my biomechanics lab — vertical ground reaction forces and impulse")
        #expect(kw == "biomechanics", "force plate data biomechanics lab should route to biomechanics")
    }
    @Test func biomechanicsKeywordFromMotionCapture() {
        let kw = CalloutManager.extractTaskKeyword(from: "processing motion capture data for my biomechanics research project on gait analysis")
        #expect(kw == "biomechanics", "motion capture biomechanics research gait analysis should route to biomechanics")
    }
    @Test func biomechanicsKeywordFromJointKinetics() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying joint kinetics and kinematics for my biomechanics exam")
        #expect(kw == "biomechanics", "joint kinetics kinematics biomechanics exam should route to biomechanics")
    }
    @Test func biomechanicsKeywordFromWord() {
        let kw = CalloutManager.extractTaskKeyword(from: "working on my biomechanics major coursework — kinematic analysis lab report")
        #expect(kw == "biomechanics", "biomechanics major kinematic analysis lab report should route to biomechanics")
    }
    @Test func biomechanicsFalsePositive_kinesiology() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying exercise physiology and strength training methods for my kinesiology exam")
        #expect(kw == "kinesiology", "exercise physiology kinesiology exam should route to kinesiology, not biomechanics")
    }
    @Test @MainActor func biomechanicsCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "biomechanics", tier: tier)
            #expect(!msgs.isEmpty, "biomechanics tier \(tier) must return messages")
        }
    }
    @Test @MainActor func biomechanicsTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "biomechanics", tier: 1)
        #expect(msgs.count >= 4, "biomechanics tier 1 must have ≥4 messages")
    }
    @Test @MainActor func biomechanicsTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "biomechanics", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "biomechanics tier 3 must contain CLOSE THIS")
    }

    // MARK: - zoology
    @Test func zoologyKeywordFromWord() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for my zoology exam — invertebrate zoology identification and taxonomy")
        #expect(kw == "zoology", "zoology exam invertebrate identification should route to zoology")
    }
    @Test func zoologyKeywordFromEntomology() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing my entomology lab report — insect morphology and taxonomic classification")
        #expect(kw == "zoology", "entomology lab insect morphology should route to zoology")
    }
    @Test func zoologyKeywordFromTaxonomy() {
        let kw = CalloutManager.extractTaskKeyword(from: "working on animal taxonomy and taxonomic classification exercises for my zoology class")
        #expect(kw == "zoology", "animal taxonomy zoology class should route to zoology")
    }
    @Test func zoologyKeywordFromWildlifeBiology() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying wildlife biology concepts for my wildlife biology major program")
        #expect(kw == "zoology", "wildlife biology major program should route to zoology")
    }
    @Test func zoologyFalsePositive_veterinary() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for the NAVLE and reviewing small animal medicine for vet school")
        #expect(kw == "veterinary", "NAVLE small animal vet school should route to veterinary, not zoology")
    }
    @Test @MainActor func zoologyCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "zoology", tier: tier)
            #expect(!msgs.isEmpty, "zoology tier \(tier) must return messages")
        }
    }
    @Test @MainActor func zoologyTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "zoology", tier: 1)
        #expect(msgs.count >= 4, "zoology tier 1 must have ≥4 messages")
    }
    @Test @MainActor func zoologyTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "zoology", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "zoology tier 3 must contain CLOSE THIS")
    }

    // MARK: - militaryscience
    @Test func militaryscienceKeywordFromROTCLab() {
        let kw = CalloutManager.extractTaskKeyword(from: "preparing for my ROTC leadership lab and reviewing cadet training requirements")
        #expect(kw == "militaryscience", "ROTC leadership lab cadet training should route to militaryscience")
    }
    @Test func militaryscienceKeywordFromMilitaryScienceClass() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for my military science class — reviewing leadership doctrine and tactics course material")
        #expect(kw == "militaryscience", "military science class leadership doctrine should route to militaryscience")
    }
    @Test func militaryscienceKeywordFromArmyROTC() {
        let kw = CalloutManager.extractTaskKeyword(from: "completing my army rotc course assignment on small unit tactics and leadership")
        #expect(kw == "militaryscience", "army rotc course assignment should route to militaryscience")
    }
    @Test func militaryscienceKeywordFromTacticsClass() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for my military tactics class final exam — maneuver warfare and decision-making")
        #expect(kw == "militaryscience", "military tactics class exam should route to militaryscience")
    }
    @Test func militaryscienceFalsePositive_militarystudies() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying military history of World War II and counterinsurgency strategy for my defense studies program")
        #expect(kw == "militarystudies", "military history counterinsurgency defense studies should route to militarystudies, not militaryscience")
    }
    @Test @MainActor func militaryscienceCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "militaryscience", tier: tier)
            #expect(!msgs.isEmpty, "militaryscience tier \(tier) must return messages")
        }
    }
    @Test @MainActor func militaryscienceTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "militaryscience", tier: 1)
        #expect(msgs.count >= 4, "militaryscience tier 1 must have ≥4 messages")
    }
    @Test @MainActor func militaryscienceTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "militaryscience", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "militaryscience tier 3 must contain CLOSE THIS")
    }

    // MARK: - healthinformatics
    @Test func healthinformaticsKeywordFromFHIR() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying FHIR standards and HL7 FHIR implementation for my health informatics class")
        #expect(kw == "healthinformatics", "FHIR standards HL7 health informatics class should route to healthinformatics")
    }
    @Test func healthinformaticsKeywordFromCPHIMS() {
        let kw = CalloutManager.extractTaskKeyword(from: "preparing for the CPHIMS exam and reviewing health data interoperability concepts")
        #expect(kw == "healthinformatics", "CPHIMS exam health data interoperability should route to healthinformatics")
    }
    @Test func healthinformaticsKeywordFromDegree() {
        let kw = CalloutManager.extractTaskKeyword(from: "working on my health informatics degree program assignment on clinical decision support systems")
        #expect(kw == "healthinformatics", "health informatics degree program clinical decision support should route to healthinformatics")
    }
    @Test func healthinformaticsKeywordFromInteroperability() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying biomedical informatics and health data exchange protocols for my informatics exam")
        #expect(kw == "healthinformatics", "biomedical informatics health data exchange informatics exam should route to healthinformatics")
    }
    @Test func healthinformaticsFalsePositive_healthcareadmin() {
        let kw = CalloutManager.extractTaskKeyword(from: "studying for the RHIA exam and reviewing health information management and hospital administration")
        #expect(kw == "healthcareadmin", "RHIA exam health information management hospital administration should route to healthcareadmin, not healthinformatics")
    }
    @Test @MainActor func healthinformaticsCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "healthinformatics", tier: tier)
            #expect(!msgs.isEmpty, "healthinformatics tier \(tier) must return messages")
        }
    }
    @Test @MainActor func healthinformaticsTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "healthinformatics", tier: 1)
        #expect(msgs.count >= 4, "healthinformatics tier 1 must have ≥4 messages")
    }
    @Test @MainActor func healthinformaticsTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "healthinformatics", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "healthinformatics tier 3 must contain CLOSE THIS")
    }
    ...
    // MARK: - nanotechnology keyword routing
    @Test func nanotechnology_quantum_dot() {
        let kw = CalloutManager.extractTaskKeyword(from: "synthesize quantum dots and characterize their optical properties for my nanotechnology lab")
        #expect(kw == "nanotechnology", "quantum dots / nanotechnology lab should route to nanotechnology")
    }
    @Test func nanotechnology_carbon_nanotube() {
        let kw = CalloutManager.extractTaskKeyword(from: "study carbon nanotube properties and write my nanotechnology assignment")
        #expect(kw == "nanotechnology", "carbon nanotubes / nanotechnology assignment should route to nanotechnology")
    }
    @Test func nanotechnology_nanomaterials() {
        let kw = CalloutManager.extractTaskKeyword(from: "characterize nanomaterials using AFM imaging for my materials class")
        #expect(kw == "nanotechnology", "nanomaterials / AFM imaging should route to nanotechnology")
    }
    @Test func nanotechnology_falsePositive_materialscience() {
        let kw = CalloutManager.extractTaskKeyword(from: "study crystallography phase diagrams and polymer science for my materials science exam")
        #expect(kw == "materialscience", "crystallography/polymer science without nano terms should stay in materialscience")
    }
    @Test @MainActor func nanotechnologyCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "nanotechnology", tier: tier)
            #expect(!msgs.isEmpty, "nanotechnology tier \(tier) must return messages")
        }
    }
    @Test @MainActor func nanotechnologyTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "nanotechnology", tier: 1)
        #expect(msgs.count >= 4, "nanotechnology tier 1 must have ≥4 messages")
    }
    @Test @MainActor func nanotechnologyTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "nanotechnology", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "nanotechnology tier 3 must contain CLOSE THIS")
    }

    // MARK: - appliedlinguistics keyword routing
    @Test func appliedlinguistics_sla() {
        let kw = CalloutManager.extractTaskKeyword(from: "write a paper on second language acquisition theory and interlanguage for my applied linguistics class")
        #expect(kw == "appliedlinguistics", "second language acquisition / applied linguistics class should route to appliedlinguistics")
    }
    @Test func appliedlinguistics_pedagogy() {
        let kw = CalloutManager.extractTaskKeyword(from: "design a language pedagogy lesson plan for language teaching methods")
        #expect(kw == "appliedlinguistics", "language pedagogy / language teaching should route to appliedlinguistics")
    }
    @Test func appliedlinguistics_policy() {
        let kw = CalloutManager.extractTaskKeyword(from: "analyze language policy and language planning for my applied linguistics research")
        #expect(kw == "appliedlinguistics", "language policy / language planning / applied linguistics research should route to appliedlinguistics")
    }
    @Test func appliedlinguistics_falsePositive_linguistics() {
        let kw = CalloutManager.extractTaskKeyword(from: "study phonology phonetics and sociolinguistics for my linguistics exam")
        #expect(kw == "linguistics", "phonology/phonetics/sociolinguistics without applied context should stay in linguistics")
    }
    @Test @MainActor func appliedlinguisticsCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "appliedlinguistics", tier: tier)
            #expect(!msgs.isEmpty, "appliedlinguistics tier \(tier) must return messages")
        }
    }
    @Test @MainActor func appliedlinguisticsTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "appliedlinguistics", tier: 1)
        #expect(msgs.count >= 4, "appliedlinguistics tier 1 must have ≥4 messages")
    }
    @Test @MainActor func appliedlinguisticsTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "appliedlinguistics", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "appliedlinguistics tier 3 must contain CLOSE THIS")
    }

    // MARK: - radiobiology keyword routing
    @Test func radiobiology_dnaDamage() {
        let kw = CalloutManager.extractTaskKeyword(from: "analyze DNA damage from ionizing radiation in my radiobiology lab")
        #expect(kw == "radiobiology", "DNA damage from radiation / radiobiology lab should route to radiobiology")
    }
    @Test func radiobiology_survivalCurve() {
        let kw = CalloutManager.extractTaskKeyword(from: "plot survival curves and analyze relative biological effectiveness for my radiobiology class")
        #expect(kw == "radiobiology", "survival curves / RBE / radiobiology class should route to radiobiology")
    }
    @Test func radiobiology_class() {
        let kw = CalloutManager.extractTaskKeyword(from: "study radiation-induced apoptosis and chromosome aberrations for my radiobiology course")
        #expect(kw == "radiobiology", "radiation-induced apoptosis / chromosome aberrations / radiobiology course should route to radiobiology")
    }
    @Test func radiobiology_falsePositive_radiologictech() {
        let kw = CalloutManager.extractTaskKeyword(from: "study ARRT exam prep and radiographic positioning for my radiologic technology program")
        #expect(kw == "radiologictechnology", "ARRT / radiographic positioning should stay in radiologictechnology")
    }
    @Test @MainActor func radiobiologyCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "radiobiology", tier: tier)
            #expect(!msgs.isEmpty, "radiobiology tier \(tier) must return messages")
        }
    }
    @Test @MainActor func radiobiologyTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "radiobiology", tier: 1)
        #expect(msgs.count >= 4, "radiobiology tier 1 must have ≥4 messages")
    }
    @Test @MainActor func radiobiologyTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "radiobiology", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "radiobiology tier 3 must contain CLOSE THIS")
    }

    // MARK: - translationstudies keyword routing
    @Test func translationstudies_literary() {
        let kw = CalloutManager.extractTaskKeyword(from: "work on my literary translation project using SDL Trados for translation class")
        #expect(kw == "translationstudies", "literary translation / Trados / translation class should route to translationstudies")
    }
    @Test func translationstudies_catTools() {
        let kw = CalloutManager.extractTaskKeyword(from: "use MemoQ for my localization project and machine translation post-editing assignment")
        #expect(kw == "translationstudies", "MemoQ / localization project / MT post-editing should route to translationstudies")
    }
    @Test func translationstudies_theory() {
        let kw = CalloutManager.extractTaskKeyword(from: "study translation theory — domestication foreignization and translation quality assessment for my translation studies course")
        #expect(kw == "translationstudies", "translation theory / translation quality assessment / translation studies course should route to translationstudies")
    }
    @Test func translationstudies_falsePositive_language() {
        let kw = CalloutManager.extractTaskKeyword(from: "practice spanish vocabulary and translate some sentences using duolingo")
        #expect(kw == "language", "bare language learning / translate sentences / duolingo should stay in language")
    }
    @Test @MainActor func translationstudiesCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "translationstudies", tier: tier)
            #expect(!msgs.isEmpty, "translationstudies tier \(tier) must return messages")
        }
    }
    @Test @MainActor func translationstudiesTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "translationstudies", tier: 1)
        #expect(msgs.count >= 4, "translationstudies tier 1 must have ≥4 messages")
    }
    @Test @MainActor func translationstudiesTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "translationstudies", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "translationstudies tier 3 must contain CLOSE THIS")
    }

    // MARK: - ecologyconservation keyword routing
    @Test func ecologyconservation_conservationBiology() {
        let kw = CalloutManager.extractTaskKeyword(from: "write a conservation biology report on habitat restoration and wildlife corridors")
        #expect(kw == "ecologyconservation", "conservation biology / habitat restoration / wildlife corridors should route to ecologyconservation")
    }
    @Test func ecologyconservation_wildlife() {
        let kw = CalloutManager.extractTaskKeyword(from: "analyze wildlife ecology data and design a species management plan for my ecology class")
        #expect(kw == "ecologyconservation", "wildlife ecology / species management / ecology class should route to ecologyconservation")
    }
    @Test func ecologyconservation_restoration() {
        let kw = CalloutManager.extractTaskKeyword(from: "study restoration ecology and landscape ecology for my conservation science research")
        #expect(kw == "ecologyconservation", "restoration ecology / landscape ecology / conservation science should route to ecologyconservation")
    }
    @Test func ecologyconservation_falsePositive_enviro() {
        let kw = CalloutManager.extractTaskKeyword(from: "study environmental science climate change and sustainability for my env sci class")
        #expect(kw == "enviro", "environmental science / climate change / sustainability should stay in enviro")
    }
    @Test @MainActor func ecologyconservationCalloutsAllTiersNonEmpty() {
        let mgr = CalloutManager()
        for tier in 1...3 {
            let msgs = mgr.taskAwareCallouts(keyword: "ecologyconservation", tier: tier)
            #expect(!msgs.isEmpty, "ecologyconservation tier \(tier) must return messages")
        }
    }
    @Test @MainActor func ecologyconservationTier1HasAtLeastFour() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "ecologyconservation", tier: 1)
        #expect(msgs.count >= 4, "ecologyconservation tier 1 must have ≥4 messages")
    }
    @Test @MainActor func ecologyconservationTier3ContainsClosethis() {
        let mgr = CalloutManager()
        let msgs = mgr.taskAwareCallouts(keyword: "ecologyconservation", tier: 3)
        #expect(msgs.contains { $0.contains("CLOSE THIS") }, "ecologyconservation tier 3 must contain CLOSE THIS")
    }

    // MARK: - Count guard (batch: nanotechnology/appliedlinguistics/radiobiology/translationstudies/ecologyconservation)
