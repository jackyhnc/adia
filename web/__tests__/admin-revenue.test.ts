...
describe('GET /api/admin/revenue — rate limit', () => {
  it('returns 429 after 20 requests from the same IP', async () => {
    const { GET } = await import('@/app/api/admin/revenue/route');
    const makeReq = () =>
      new NextRequest('http://localhost/api/admin/revenue', {
        method: 'GET',
        headers: {
          Authorization: 'Bearer test-admin-token',
          'x-forwarded-for': '10.91.3.1',
        },
      });
